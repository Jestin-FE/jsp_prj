...
public class BoardUtil {
	...
	public static String pagination(PaginationDTO pDTO) {
		...
		// int pDTO.getPageNumber() = 3; // 한 화면에 보여줄 페이지 인덱스의 수
		// 2. 화면에 보여줄 시작페이지 번호
		...
		if (pDTO.getCurrentPage() > pDTO.getPageNumber()) { // 시작페이지보다 1 작은 페이지로 이동
			...
			.append("' class='pagiPrevMark'>&lt;&lt;</a></span> ]");
		} // end if
		...
		while (movePage <= endPage) {  // 
			if (movePage == pDTO.getCurrentPage()) { // 현재 페이지는 링크를 설정하지 않음.
				...
				pageLink.append("[ <a href='").append(pDTO.getUrl()).append("?currentPage=")
					.append(movePage)
					.append(searchQueryString.toString())
					.append("' class='pagiNotCurrent'>").append(movePage).append("</a></span> ]");
		...
		// 7. 뒤에 페이지가 더 있는 경우
		StringBuilder nextMark = new StringBuilder("[ <span class='pagiNextMark'>&gt;&gt; ]");
		if (pDTO.getTotalPage() > endPage) {
