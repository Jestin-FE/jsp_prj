...
for (int i = 0; i<blockIp.length; i++) {
	if (ip.endsWith(blockIp[i])) {
		...
		return; // _jspService라는 메소드 안에 코드가 들어가므로 return 사용 가능!
	} // end if
} // end for
%>
...
<li><span>요청 서블릿명 : </span><%= request.getServletPath() %></li>
...
<div>
<!--
	use_request.jsp를 요청하면서 다음 파라미터로 구성해보시오.
	name=test, age=20 
-->
<a href="use_request.jsp?name=test&age=20">GET방식의 요청</a> <!-- ""이면 -->
</div>
<div>
<form action="use_request.jsp" method="post" id="frm" name="frm">
<input type="hidden" name="name" value="test"> <!-- 고정된 값은 hidden으로 처리 -->
