...
<style type="text/css">
 ...
 th {
 	background-color: #eff !important; /* !important를 붙이면 Bootstrap 스타일보다 우선 */
 }
...
<%
	...
	int finalDate = ld.lengthOfMonth();
	int startDayOfWeek = ld.withDayOfMonth(1).getDayOfWeek().getValue();
	int start = startDayOfWeek % 7;
	int num = 1;
%>
<div class="ym">
<%= nowYear %>-<%= nowMonth %>
...
%>				
				<td class="td_<%= j %>"></td>
<%			} else if (num <= finalDate) { %>
				<td class="td_<%= j %>"><%= num %></td>
<%			
				num++;
			} else {
%>
