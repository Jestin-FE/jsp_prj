...
$(function(){
	...
	$("#btnUpdate").click(function() {
		...
		if (confirm("회원정보를 수정하시겠습니까?")) {
			...
			var frm = $("#frm")[0]; // parameter 전송 방식 : 파일업로드가 되지 않음.
			var formData = new FormData(frm); // binary 전송방식이 되어 파일업로드가 가능해짐
			...
			$.ajax({
				...
				success: function(jsonObj) {
					...
					console.log("프로필 이미지가 업로드되지 않았습니다.");
					} // else
				}
			}); // ajax
		} // 이미지 업로드
...
if (mDTO == null) {
	response.sendRedirect("http://localhost/jsp_prj"); // index.html이 자동으로 불려짐.
	return;	
} // end if
...
<tr>
<td>가입 시 사용된 ip address </td>
<td><c:out value="${mDTO.ip}"></c:out></td>
</tr>
<tr>
<td>가입일 </td>
